...
class XI(torch.nn.Module):
    def __init__(self, in_dim, hidden_size=256, stddev=False, train_mean=True, train_prec=True, **kwargs):
        ...
        self.prior_mean = torch.nn.Parameter(torch.zeros(1, self.input_dim), requires_grad=train_mean)
        self.prior_logprec = torch.nn.Parameter(torch.zeros(1, self.input_dim), requires_grad=train_prec)
        ...
        self.lin1_relu_bn = nn.Sequential(
                ...
                nn.BatchNorm1d(hidden_size))  # tc: change BN->activation to act -> BN
        self.lin2 = nn.Conv1d(hidden_size, self.input_dim, kernel_size=1, stride=1, bias=True)
    ...
    def forward(self, inputs):
        ...
        if self.stddev:
            ...
            sigma = torch.sqrt(torch.clamp(sigma2 - phi ** 2, min=1.0e-12))  # tc: min change from 1.0e-10 to 1.0e-12 (follow speechbrain default)
