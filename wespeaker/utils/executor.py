    ...
    with torch.set_grad_enabled(True):
        for i, (utts, features, targets) in enumerate(dataloader):
            ...
            if i % log_batch_interval == 0:
                logger.info(tp.row((epoch, i, scheduler.get_lr(), margin_scheduler.get_margin())+(loss_meter.value()[0], acc_meter.value()[0]), width=10, style='grid'))
    
    logger.info(tp.row((epoch, len(dataloader), scheduler.get_lr(), margin_scheduler.get_margin())+(loss_meter.value()[0], acc_meter.value()[0]), width=10, style='grid'))
