...
def comput_matmul(x,y):
    y = np.mean(y, axis=1, keepdims=True)
    x_reshaped = x.reshape(x.shape[0] * x.shape[1], x.shape[2])
    y_reshaped = y.reshape(y.shape[0] * y.shape[1], y.shape[2])
    
    sim = cosine_similarity(x_reshaped, y_reshaped)
    
    sim_reshaped = sim.reshape(x.shape[0], x.shape[1], y.shape[0], y.shape[1])
    
    final_sim = sim_reshaped.mean(axis=1).mean(axis=-1)
...
def get_mean_std(emb, cohort, top_n):
    ...
    emb_cohort_score = comput_matmul(emb, cohort)
    # print(emb_cohort_score[0])
