...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  for dset in vox2_dev vox1; do
    if [ ! -f ${data}/${dset}/dur.scp ]; then
      python tools/wav2dur.py ${data}/${dset}/wav.scp ${data}/${dset}/dur.scp > ${data}/${dset}/dur.log
    fi
  done
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  cat ${data}/vox1/dur.scp ${data}/vox2_dev/dur.scp > ${exp_dir}/scores/calibration/dur.scp
  for x in ${calibration_trial} $trials; do
    python wespeaker/bin/score_calibration.py "gather_calibration_factors" \
      --wav_dur_scp ${exp_dir}/scores/calibration/dur.scp \
      --max_dur 20 \
