...
exp_dir=exp/ResNet34-TSTP-emb256-fbank80-num_frms200-cnceleb-aug0.6-spTrue-saFalse-ArcMargin-SGD-epoch150_UIO
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
  ...
  cat data/cnceleb_train/utt2spk | awk '{print $2}' | sort | uniq | \
      awk '{print $1, NR - 1}' > data/cnceleb_train/spk2id
fi
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  if [ $data_type == "shard" ]; then
    python tools/make_shard_list.py --num_utts_per_shard 1000 \
        ...
        --seed '777' \
        --shuffle \
        data/cnceleb_train/wav.scp data/cnceleb_train/utt2spk \
        data/cnceleb_train/shards data/cnceleb_train/shard.list
  else
    python tools/make_raw_list.py data/cnceleb_train/wav.scp \
        data/cnceleb_train/utt2spk data/cnceleb_train/raw.list
  fi
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --standalone --nnodes=1 --nproc_per_node=$num_gpus \
    wespeaker/bin/train_uio.py --config $config \
      ...
      --num_avg ${num_avg} \
      --data_type "${data_type}" \
      --train_list data/cnceleb_train/${data_type}.list \
      --spk2id data/cnceleb_train/spk2id \
