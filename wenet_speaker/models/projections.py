...
def get_projection(conf):
    if conf['project_type'] == 'add_margin':
        projection = AddMarginProduct(conf['embed_dim'], conf['num_class'], scale=conf['scale'], margin=0.0)
    elif conf['project_type'] == 'arc_margin':
        projection = ArcMarginProduct(conf['embed_dim'], conf['num_class'], scale=conf['scale'], margin=0.0, easy_margin=conf['easy_margin'])
    elif conf['project_type'] == 'sphere':
        projection = SphereProduct(conf['embed_dim'], conf['num_class'], margin=4)
    else:
...
#        self.scale = self.scale
    ...
    def extra_repr(self):
        return 'in_features={}, out_features={}, scale={}, margin={}, easy_margin={}'.format(
            self.in_features, self.out_features, self.scale, self.margin, self.easy_margin
        )
...
if __name__ == '__main__':
    projection = ArcMarginProduct(100, 200, scale=32.0, margin=0.2, easy_margin=False)
